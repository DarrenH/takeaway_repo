...
class HTTP
{
	...
	function getData($url, $data = '',$stop = false)
	{
		$url = $this->path . $url;
		if(is_array($data))
		{
...
class cPanel
{
  ...
  */
	...
	{
		$accountList = array();
		preg_match_all('/\?user=([^"]*&)/', $this->HTTP->getData('mail/pops_noscript.html'), $accountList);
		if(count($accountList[1]) > 0)
		{
			return rtrim($accountList[1][0], '&');
		}
		return false;
	}
  ...
  */
	...
	{
		$databaseList = array();
		preg_match_all('/deldbconfirm.html\?db=([^"]*)/', $this->HTTP->getData('sql/index.html'), $databaseList);
...
class Database
{
  ...
  */
	function create()
	{
		...
		$response = $this->HTTP->getData('sql/addb.html', $data);
